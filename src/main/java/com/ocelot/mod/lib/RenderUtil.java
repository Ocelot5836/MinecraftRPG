...
public class RenderUtil {
	...
	private static boolean[] cullFaces = new boolean[EnumFacing.values().length];
	...
	private static void renderModel(IBakedModel model, @Nullable Transform transform, ItemStack stack) {
		...
		for (EnumFacing enumfacing : EnumFacing.values()) {
			if (!cullFaces[enumfacing.getIndex()]) {
	...
	public static void cull(EnumFacing facing, boolean cull) {
		cullFaces[facing.getIndex()] = cull;
	}
}